...
class FileTest {
    ...
    private fun jsonEquals(x1: JSONObject, x2: JSONObject): Boolean {
        if (x1.size() != x2.size()) {
            return false
        }
        val names = x1.keySet()
        for (name in names) {
            if (!jsonEquals(x1[name], x2[name])) {
                return false
            }
        }
        return true
    ...
    private fun jsonEquals(element1: Any?, element2: Any?): Boolean {
        if (element1?.javaClass != element2?.javaClass) {
            return false
        }
